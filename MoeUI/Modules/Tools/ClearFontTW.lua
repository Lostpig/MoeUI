...
-- 任务说明和书信、石碑的正文字体
local CLEAR_FONT_QUEST = CLEAR_FONT_BASE.."bKAI00M.TTF"
-- 战斗伤害数值提示
local CLEAR_FONT_DAMAGE = CLEAR_FONT_BASE.."bHEI01B.TTF"
-- 游戏界面中的主要字体
local CLEAR_FONT = CLEAR_FONT_BASE.."bLEI00D.TTF"
-- 物品、技能的说明字体
local CLEAR_FONT_ITEM = CLEAR_FONT_BASE.."bKAI00M.TTF"
-- 聊天字体
local CLEAR_FONT_CHAT = CLEAR_FONT_BASE.."bLEI00D.TTF"
...
-- 詳細榮譽點、競技場比分（PvP面板），時間資訊，系統功能表子專案
	if (CanSetFont(GameFontHighlightSmall)) then 		GameFontHighlightSmall:SetFont(CLEAR_FONT, 14 * CF_SCALE); end	-- 預設值：15
	if (CanSetFont(GameFontHighlightSmallOutline)) then	GameFontHighlightSmallOutline:SetFont(CLEAR_FONT, 14 * CF_SCALE, "OUTLINE"); end
...
-- 任務描述
	...
	if (CanSetFont(QuestFont)) then 		   			QuestFont:SetTextColor(0.18, 0.12, 0.06); end			-- 預設值：(0, 0, 0)
